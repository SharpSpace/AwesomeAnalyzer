...
namespace AwesomeAnalyzer.Analyzers
{
    ...
    public sealed class EnumerateQueryableMultipleTimesAnalyzer : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics => ImmutableArray.Create(
            DiagnosticDescriptors.Rule0011EnumerateQueryableMultipleTimes
        );
        ...
        private static void AnalyzeNode(SyntaxNodeAnalysisContext context)
        {
            ...
            {
                if (context.IsDisabledEditorConfig(DiagnosticDescriptors.Rule0011EnumerateQueryableMultipleTimes.Id))
                {
                    return;
                }
                ...
                // Collect candidate variables: IQueryable<T> locals with non-materialized initializers.
                ...
                foreach (var variable in localDeclaration.Declaration.Variables)
                {
                    ...
                    if (!IsQueryableType(symbol.Type, context.SemanticModel.Compilation)) continue;
        ...
        private static bool IsQueryableType(ITypeSymbol type, Compilation compilation)
        {
            if (!(type is INamedTypeSymbol namedType)) return false;

            var genericQueryable = compilation.GetTypeByMetadataName("System.Linq.IQueryable`1");
            if (genericQueryable == null) return false;

            return SymbolEqualityComparer.Default.Equals(namedType.OriginalDefinition, genericQueryable);
        }
